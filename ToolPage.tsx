...
function ToolPage() {
  ...
  if (!toolId) {
    const title = `${category.name} Tools - Dicetools`;
    const description = `Explore all ${category.name} tools available on Dicetools. ${category.description}`;
    const canonicalUrl = `https://dicetools.online/${categoryId}`;

    return (
      <>
        <SeoContent title={title} description={description} canonicalUrl={canonicalUrl} />
